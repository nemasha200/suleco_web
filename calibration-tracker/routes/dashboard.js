...
router.get('/', (req, res) => {
  ...
  const dueSoonCount = withBadges.filter(r => r.daysLeft !== null && r.daysLeft >= 0 && r.daysLeft <= 14).length;
...
router.get('/export.csv', (req, res) => {
  ...
  const header = 'Customer,Phone,Email,Brand,Model,Serial Number,Last Calibration,Next Calibration,Status\n';
  const csv = rows.map(r => [
    r.customer_name, r.phone, r.email, r.brand, r.model, r.serial_number,
...
router.get('/notifications', (req, res) => {
  ...
  `).all();
  res.render('notifications', { logs, username: req.session.username });
});
